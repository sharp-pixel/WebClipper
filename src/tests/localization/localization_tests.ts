...
QUnit.test("getLocalizedString returns the localized string if there is a matching key", () => {
...
QUnit.test("getLocalizedString returns the backup string if there is no matching key", () => {
	let mockLocalizedStrings = getMockLocalizedStrings();
...
QUnit.test("getLocalizedString returns the backup string if there is no localized strings populated", () => {
	strictEqual(Localization.getLocalizedString(existKey), backupStrings[existKey],
		"If no localized strings exist, the backup string should be fetched");
});

QUnit.test("getLocalizedString should throw an error if there is no matching key in the localized or backup strings", () => {
	throws(() => {
		let mockLocalizedStrings = getMockLocalizedStrings();
		Localization.setLocalizedStrings(mockLocalizedStrings);
...
QUnit.test("getLocalizedString should throw an error if there is no matching key in the backup strings and no localized strings were set", () => {
	throws(() => {
		Localization.getLocalizedString("Non.Existing.Key");
	}, Error("getLocalizedString could not find a localized or fallback string: Non.Existing.Key"));
});

QUnit.test("getLocalizedString should throw an error if the parameter is the empty string", () => {
	throws(() => {
		Localization.getLocalizedString("");
	}, Error("stringId must be a non-empty string, but was: "));
});

QUnit.test("getLocalizedString should throw an error if the parameter is null", () => {
...
QUnit.test("getLocalizedString should throw an error if the parameter is undefined", () => {
	throws(() => {
		Localization.getLocalizedString(undefined);
	}, Error("stringId must be a non-empty string, but was: undefined"));
});

QUnit.test("formatFontFamily should ensure that font family names that contain inner-spaces should be wrapped in single quotes", () => {
...
QUnit.test("formatFontFamily should not format an already-formatted input, but should simply return the input", () => {
	let fontFamily = "'Segoe UI Bold','Segoe UI',Segoe,'Segoe WP','Helvetica Neue',Roboto,Helvetica,Arial,Tahoma,Verdana,sans-serif";
	let formattedFontFamily = Localization.formatFontFamily(fontFamily);
	strictEqual(formattedFontFamily, fontFamily, "Already-formatted values are left as-is");
});

QUnit.test("formatFontFamily should correctly format an inner-space containing name if it is the only one in the input", () => {
...
QUnit.test("formatFontFamily should leave an already-formatted name as-is if it is the only one in the input", () => {
...
QUnit.test("formatFontFamily should trim names with unecessary whitespace", () => {
...
QUnit.test("formatFontFamily should return empty string when passed an empty string", () => {
...
QUnit.test("formatFontFamily should return empty string when passed null", () => {
	/* tslint:disable:no-null-keyword */
...
QUnit.test("formatFontFamily should return empty string when passed undefined", () => {
