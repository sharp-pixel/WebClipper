...
QUnit.test("settings.json object should be non-empty when required from any project", () => {
...
QUnit.test("helper function checkValueAndDescriptionOnObject should assert when given an object that lacks a value or description", () => {
	let nothing = {
		"Dummy": "blah"
	};
...
QUnit.test("All entries in setting.json should have a non-empty Value and Description", () => {
...
QUnit.test("GetSetting should return undefined when passed the empty String as a key, because no setting can be named the empty string", () => {
	let value = Settings.getSetting("");
	strictEqual(value, undefined, "Blank key for Settings.GetSetting returned blank key back");
});

QUnit.test("GetSetting should return the key that was passed in when it cannot find the setting in settings.json", () => {
...
QUnit.test("GetSetting should correctly fetch the Value of a setting when given a valid Key", () => {
